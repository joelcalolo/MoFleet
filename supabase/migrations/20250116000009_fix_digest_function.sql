...
BEGIN
  ...
  -- Se não estiver, você precisa habilitá-la manualmente no Dashboard:
  -- Database > Extensions > pgcrypto > Enable
  BEGIN
    -- Tentar usar SHA-256 via pgcrypto
    admin_password_hash := encode(digest(admin_password, 'sha256'), 'hex');
  EXCEPTION
    WHEN undefined_function THEN
      -- Se digest() não existir, a extensão pgcrypto não está disponível
